...
class SlowFast(nn.Module):
    def __init__(self, block=Bottleneck, alpha=4, beta=8, layers=[3, 4, 6, 3], class_num=10, dropout=0.5 ):
        super(SlowFast, self).__init__()
        self.alpha = alpha
        self.fast_inplanes = 64//beta
        self.fast_conv1 = nn.Conv3d(3, 64//beta, kernel_size=(5, 7, 7), stride=(1, 2, 2), padding=(2, 3, 3), bias=False)
        self.fast_bn1 = nn.BatchNorm3d(64//beta)
        self.fast_relu = nn.ReLU(inplace=True)
        self.fast_maxpool = nn.MaxPool3d(kernel_size=(1, 3, 3), stride=(1, 2, 2), padding=(0, 1, 1))
        self.fast_res2 = self._make_layer_fast(block, 64//beta, layers[0], head_conv=3)
        self.fast_res3 = self._make_layer_fast(
            block, 128//beta, layers[1], stride=2, head_conv=3)
        self.fast_res4 = self._make_layer_fast(
            block, 256//beta, layers[2], stride=2, head_conv=3)
        self.fast_res5 = self._make_layer_fast(
            block, 512//beta, layers[3], stride=2, head_conv=3)
        
        self.lateral_p1 = nn.Conv3d(64//beta, 64//beta*2, kernel_size=(5, 1, 1), stride=(alpha, 1 ,1), bias=False, padding=(2, 0, 0))
        self.lateral_res2 = nn.Conv3d(256//beta, 256//beta*2, kernel_size=(5, 1, 1), stride=(alpha, 1 ,1), bias=False, padding=(2, 0, 0))
        self.lateral_res3 = nn.Conv3d(512//beta, 512//beta*2, kernel_size=(5, 1, 1), stride=(alpha, 1 ,1), bias=False, padding=(2, 0, 0))
        self.lateral_res4 = nn.Conv3d(1024//beta, 1024//beta*2, kernel_size=(5, 1, 1), stride=(alpha, 1 ,1), bias=False, padding=(2, 0, 0))

        self.slow_inplanes = 64+64//alpha*1
    ...
    def forward(self, input):
        input = input.permute(0,2,1,3,4) # from [-1, C, T, H, W] to [-1, T, C, H, W]
        fast, lateral = self.FastPath(input[:, :, ::1, :, :])
        slow = self.SlowPath(input[:, :, ::self.alpha*1, :, :], lateral)
...
def generate_model(resnet_version='resnet50', alpha=4, class_num=3):
    if resnet_version=='resnet50':
        # Constructs a ResNet-50 model.
        model = SlowFast(Bottleneck, alpha=alpha, layers=[3, 4, 6, 3], class_num=3)
    elif resnet_version=='resnet101':
        #Constructs a ResNet-101 model.
        model = SlowFast(Bottleneck, alpha=alpha, layers=[3, 4, 23, 3], class_num=3)
    elif resnet_version=='resnet152':
        # Constructs a ResNet-152 model.
        model = SlowFast(Bottleneck, alpha=alpha, layers=[3, 8, 36, 3], class_num=3)
    elif resnet_version=='resnet200':
        # Constructs a ResNet-200 model.
        model = SlowFast(Bottleneck, alpha=alpha, layers=[3, 24, 36, 3], class_num=3)
    return model
