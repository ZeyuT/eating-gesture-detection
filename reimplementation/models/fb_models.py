...
class SlowFast(nn.Module):
    ...
    def __init__(self, cfg_file):
        ...
        The `__init__` method of any subclass should also contain these
            arguments.
        Args:
            cfg_file: file path for model building configs
        """
    ...
    def split_paths(self,x):
        # from [-1, T, C, H, W] to [-1, C, T, H, W] 
        x = x.permute(0,2,1,3,4) 
        # the dataset is sampled in 16Hz, same as the fastway's in the 8*8 model
        fast_pathway = x[:, :, ::1, :, :] 
        # resample to 16//self.alpha (i.e. 4) Hz, to fit in the 8*8 model
        slow_pathway = x[:, :, ::16//self.alpha, :, :] 
        return [slow_pathway,fast_pathway]
    
    def forward(self, x, bboxes=None):
        x = self.split_paths(x)
        x = self.s1(x)
        x = self.s1_fuse(x)
        x = self.s2(x)
        x = self.s2_fuse(x)
...
class X3D(nn.Module):
    ...
    def forward(self, x, bboxes=None):
        x = torch.unsqueeze(x,dim=0) # [new,-1, T, C, H, W]
        x = x.permute(0,1,3,2,4,5) # from [new, -1, T, C, H, W] to [-1, new, C, T, H, W] 
