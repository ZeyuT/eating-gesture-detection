...
from constants import WIDTH,HEIGHT,FRAME_LOC
...
def process_frames(args): 
    window_loc, frame_save_loc, gesture_gt_loc, video_sync_offset, fps = args[0],args[1],args[2],args[3],args[4]      
    ...
    for frame_name in frame_names:
        ...
        count += 1
        timestep += int(1 / fps * 1000)        
    print("{} finished: processed {} images".format(frame_save_loc,count), flush=True)
    #print("--------------------------------------------", flush=True)
    f_gt_frame.close()
    return count
            ...
            if cur_start > pre_end + 1:
                ...
                while (1):
                    ...
                    if drink_locations[drink_idx] < unlabel_start:
                        ...
                        break
                if len(bite_locations) != 0 and unlabel_start <= bite_locations[bite_idx] <= unlabel_end:
                    if timeidx_to_ms(bite_locations[bite_idx]) - PRE_INTAKE_DURATION > timeidx_to_ms(unlabel_start):                  
                        gesture_types.append("non_intake")
                        gesture_start.append(timeidx_to_ms(unlabel_start))
                        gesture_end.append(timeidx_to_ms(bite_locations[bite_idx]-1) - PRE_INTAKE_DURATION)
                    gesture_types.append("bite")          
                    gesture_start.append(max(timeidx_to_ms(bite_locations[bite_idx]) - PRE_INTAKE_DURATION, timeidx_to_ms(unlabel_start))) # data index of start of gesture
                    gesture_end.append(min(timeidx_to_ms(bite_locations[bite_idx]) + AFTER_INTAKE_DURATION, timeidx_to_ms(unlabel_end)))  # data index of end of gesture   
                    if timeidx_to_ms(bite_locations[bite_idx]) + AFTER_INTAKE_DURATION < timeidx_to_ms(unlabel_end):                  
                        gesture_types.append("non_intake")
                        gesture_start.append(timeidx_to_ms(bite_locations[bite_idx]+1) + AFTER_INTAKE_DURATION)
                        gesture_end.append(timeidx_to_ms(unlabel_end))        
                elif len(drink_locations) != 0 and unlabel_start <= drink_locations[drink_idx] <= unlabel_end:
                    if timeidx_to_ms(drink_locations[drink_idx]) - PRE_INTAKE_DURATION > timeidx_to_ms(unlabel_start):                  
                        gesture_types.append("non_intake")
                        gesture_start.append(timeidx_to_ms(unlabel_start))
                        gesture_end.append(timeidx_to_ms(drink_locations[drink_idx]-1) - PRE_INTAKE_DURATION )
                    gesture_types.append("drink")
                    gesture_start.append(max(timeidx_to_ms(drink_locations[drink_idx]) - PRE_INTAKE_DURATION, timeidx_to_ms(unlabel_start))) # data index of start of gesture
                    gesture_end.append(min(timeidx_to_ms(drink_locations[drink_idx]) + AFTER_INTAKE_DURATION, timeidx_to_ms(unlabel_end)))  # data index of end of gesture 
                    if timeidx_to_ms(drink_locations[drink_idx]) + AFTER_INTAKE_DURATION < timeidx_to_ms(unlabel_end):    
                        gesture_types.append("non_intake")
                        gesture_start.append(timeidx_to_ms(drink_locations[drink_idx]+1) + AFTER_INTAKE_DURATION)
                        gesture_end.append(timeidx_to_ms(unlabel_end)) 
                else:
                    gesture_start.append(timeidx_to_ms(unlabel_start)) # data index of start of gesture
                    gesture_end.append(timeidx_to_ms(unlabel_end))	  # data index of end of gesture
        ...
        if label in ["rest","other"]:
            ...
            while (1):
                ...
                if drink_locations[drink_idx] < cur_start:
                    ...
                    break
            if len(bite_locations) != 0 and cur_start <= bite_locations[bite_idx] <= cur_end:
                if timeidx_to_ms(bite_locations[bite_idx]) - PRE_INTAKE_DURATION > timeidx_to_ms(cur_start):                  
                    gesture_types.append("non_intake")
                    gesture_start.append(timeidx_to_ms(cur_start))
                    gesture_end.append(timeidx_to_ms(bite_locations[bite_idx]-1) - PRE_INTAKE_DURATION)
                gesture_types.append("bite")          
                ...
                if timeidx_to_ms(bite_locations[bite_idx]) + AFTER_INTAKE_DURATION < timeidx_to_ms(cur_end):                  
                    gesture_types.append("non_intake")
                    gesture_start.append(timeidx_to_ms(bite_locations[bite_idx]+1) + AFTER_INTAKE_DURATION)
                    gesture_end.append(timeidx_to_ms(cur_end))      

            elif len(drink_locations) != 0 and cur_start <= drink_locations[drink_idx] <= cur_end:
                print(cur_start,cur_end,drink_locations[drink_idx])
                if timeidx_to_ms(drink_locations[drink_idx]) - PRE_INTAKE_DURATION > timeidx_to_ms(cur_start):                  
                    gesture_types.append("non_intake")
                    gesture_start.append(timeidx_to_ms(cur_start))
                    gesture_end.append(timeidx_to_ms(drink_locations[drink_idx]-1) - PRE_INTAKE_DURATION )
                gesture_types.append("drink")
                ...
                if timeidx_to_ms(drink_locations[drink_idx]) + AFTER_INTAKE_DURATION < timeidx_to_ms(cur_end):    
...
if __name__ == "__main__": 
    ...
    f_filelist.close()
    #filelists = ["p176/c1/20120328132824062.txt"]
    '''
