...
from constants import LABEL_TABLE,LABEL_NUM
...
def main():       
    ...
    if first_network == 'RES_BILSTM':
        fps = 8
        downsample_rate = fps // 4 # i.e. downsample to 4 hz
        raw_sample_len = 8400 # 8400/8 = 1050 s
        raw_seq_len = 16
        sample_len = int(raw_sample_len/downsample_rate)
    elif first_network == 'x3d-s':
        fps = 5
        downsample_rate = fps // 4 # i.e. downsample to 4 hz
        raw_sample_len = 5250 # 1050 s
        raw_seq_len = 13
        sample_len = int(raw_sample_len/downsample_rate)

    raw_data_loc = f"/scratch/zeyut/EatSense/VideoData_eatSense_{fps}hz/"
    feat_data_loc = f"/zfs/mhealth/zeyut/eat_detection/EatSense/results_10runs_{first_network}/"    
    log_loc = os.path.join(feat_data_loc,'global_detector',f"log_{network}_{first_network}_{raw_seq_len}_2stage_{label_type}")
    model_loc = os.path.join(feat_data_loc,'global_detector',f"model_{network}_{first_network}_{raw_seq_len}_2stage_{label_type}")
    test_loc = os.path.join(feat_data_loc,'global_detector',f"result_{network}_{first_network}_{raw_seq_len}_2stage_{label_type}")  
    ...
    if train != 0:    
        ...
        train_set = probabilityDataset(
                    data_loc = feat_data_loc,
                    gt_loc = os.path.join(raw_data_loc,"train_set"),
                    mode = "train",
                    label_type = label_type,
                    run_times = 10,
                    raw_seq_len = raw_seq_len,
        ...
        val_set = probabilityDataset(
                    data_loc = feat_data_loc,
                    gt_loc = os.path.join(raw_data_loc,"val_set"),
                    mode = "val",
                    label_type = label_type,
                    run_times = 10,
                    raw_seq_len = raw_seq_len,
    ...
    mean_list = np.loadtxt(os.path.join(feat_data_loc,f"train_mean_{label_type}.txt"))
    std_list = np.loadtxt(os.path.join(feat_data_loc,f"train_std_{label_type}.txt"))
    
    for model_idx in range(1,11):
        test_2nd_stage(model = model,
                        data_loc = os.path.join(feat_data_loc,f"{model_idx}/result_test/frame_features/"), 
                        gt_loc = os.path.join(raw_data_loc,"test_set"),
