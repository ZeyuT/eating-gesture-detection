...
def main():   
    ...
    if train == 5:
        ...
        log_loc = f"./log_{network}_{30+epochs}_{seq_len}_{stride}_v{weight_type}"
        model_loc = f"./model_{network}_{30+epochs}_{seq_len}_{stride}_v{weight_type}"
        test_loc = f"./test_{network}_{30+epochs}_{seq_len}_{stride}_v{weight_type}"
    else:
    ...
    if train == 2:
        train_video_list = [train_video_list[0]]
        val_video_list = [val_video_list[0]]
        test_video_list = [test_video_list[0]]
    ...
    sys.stdout.flush()
    log.flush()
    weights = np.array([0.27737257, 0.67551568, 0.04711176])
